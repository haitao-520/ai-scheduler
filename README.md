...
- 图片识别需要选择支持视觉的模型（如 `deepseek-vl` 系列），默认的 `deepseek-chat` 不支持图片输入。
- 应用内已内置常见班次的默认时间（早班 08:00-12:40、中班 12:40-17:20、晚班 17:20-22:00、夜班 22:00-08:00、白班 08:00-18:00）。
...
发布约定：每次构建递增 `android/app/build.gradle` 中的 `versionCode`（加 1）与 `versionName`（加 0.01）。当前版本为 `1.17`（versionCode 18）。
